...
def get_view(
    ...
    resolution_image = images[0].shape[0]
    resolution_3d = intrinsics_dict[0]['resolution_3d']
    imaging_i = RaycastingImaging(resolution_image=resolution_image, 
...
def pairwise_interpolate_predictions(
    ...
    uv_i = imaging_i.rays_origins[:, :-1]
    points_to_query = reprojected_j[:, :-1]
    _, nn_indexes_in_i = cKDTree(uv_i).query(x=points_to_query, k=nn_set_size)
    ...
    flatten_distance_i = distances_i.flatten()
    ...
    for idx, point_from_j in (enumerate(reprojected_j)):
        ...
        xy_points = uv_i[point_nn_indexes]
        z_points = flatten_image_i[point_nn_indexes].reshape(-1, 1)

        # print(xy_points.shape, z_points.shape)

        point_from_j_nns = np.hstack((xy_points, z_points))
        ...
        residuals_sum = np.sum((point_from_j_nns - point_from_j) ** 2, axis=1)
        distances_to_nearest = np.sqrt(residuals_sum)

        mask = distance_interpolation_threshold > distances_to_nearest
        interp_mask[idx] = np.all(mask)
        ...
        if interp_mask[idx]:
            ...
            try:
                ...
                interpolator = interpolate.interp2d(
                  x=point_from_j_nns[:, 0],
                  y=point_from_j_nns[:, 1],
                  z=flatten_distance_i[point_nn_indexes]
                )

                xnew = point_from_j[0]
                ynew = point_from_j[1]
                distances_j_interp[idx] = interpolator(xnew, ynew)
